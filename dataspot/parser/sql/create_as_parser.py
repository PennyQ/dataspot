...
class CreateAsParser:
    ...
    def set_length(self, parser):
        self._length = len(parser['create'][0]) - 1
    ...
    def find_creates(self, parser):
        ...
        if self._count <= self._length:
            ...
            if self._count == 0 and self._lines[0].lower().find(parser['create'][0][self._count]) != -1:
                ...
                # next line
                if self._count == 1 and self._start_found == 1 and self._query.find(parser['create'][0][self._count]) == -1:
                    # If the second item is in the next line then we know that we have the type of statement we are
                    # after in this parser.
                    if self._lines[0].lower().find(parser['create'][0][self._count]) != -1:
                        self._query += self._lines[0].lower()
                        self._count += 1
                        self._lines.pop(0)
                ...
                elif self._count == 1 and self._start_found == 1 and self._query.find(parser['create'][0][self._count]) != -1:
                    self._count += 1
                return self._query
            ...
            # we can use the statements below.
            elif self._lines[0].lower().find(parser['create'][0][self._count]) == -1 and self._start_found == 1:
                self._query += self._lines[0].lower()
                self._lines.pop(0)
                ...
                if self._query.find(parser['create'][1][0]) != -1:
                    self._query = ""
                    self._count = 0
                    self._end_found = 0
                    self._start_found = 0
        ...
        # When we have found all of the items in the list, we want to find the closing item of the query.
            ...
            elif self._lines[0].lower().find(parser['create'][1][0]) != -1:
                self._query += self._lines[0].lower()
                self._lines.pop(0)
                self._end_found = 1
                return self._query
